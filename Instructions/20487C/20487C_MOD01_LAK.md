...
6. Locate the **&lt;configuration&gt;** section. Add a **&lt;connectionStrings&gt;&lt;/connectionStrings&gt;** element under it.
7. Place the cursor after the **&lt;connectionStrings&gt;** tag, and then press Ctrl+V to paste the connection string.
8. To save the changes, press Ctrl+S.
9. On the **Build** menu, click **Build Solution**.
10. On the **View** menu, click **Server Explorer**.
11. In **Server Explorer**, right-click **Data Connections**, and then click **Refresh**. **BlueYonderEntities** displays under the **Data Connections** node.
12. In **Solution Explorer**, in the **BlueYonder.MVC** project, right-click **Controllers**, point to **Add**, and then click **Controller**.
13. In the  **Add Scaffold** dialog box, in the navigation pane, click **Controller**, from the **Template** list select **Web API 2 controller with actions, using Entity Framework** and then click **Add**. 
14. In the **Add Controller** dialog box, from the **Model class** combo box, select **Location(BlueYonder.Model)**
15. In the **Data context class** combo box, select **BlueYonderEntities (BlueYonder.Model)**.
16. In the **Controller name** combo box, type **LocationsController**.
17. To create the controller, click **Add**.

    >**Note:** You now have a Web API controller for the Location model.

18. In **Solution Explorer**, right-click the **BlueYonder.MVC** project, and then click **Set as StartUp Project**.
19. To start the application without debugging, press Ctrl+F5.
20. A browser window opens, in the address bar, append **api/locations** to the address, and then press Enter.
21. On Google Chrome, XML will appear. On Microsoft Edge and Mozilla Firefox, JSON will appear, and on Internet Explorer, a file download will be prompted.
