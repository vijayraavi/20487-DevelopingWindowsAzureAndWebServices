...
10. In **Server Explorer**, right-click **Data Connections**, and then click **Refresh**. **BlueYonderEntities** displays under the **Data Connections** node.
...
19. A browser window opens, in the address bar, append **api/locations** to the address, and then press Enter.
20. On Google Chrome, XML will appear, on Microsoft Edge and Mozilla Firefox, will appear, and on Internet Explorer, a file download will be prompted.
...
3. Click **Publish**.
4. An **App Service** window opens. If you are logged in to Visual Studio, your account might appear on the top right corner of the window. If it doesn't, click the **Add an Account** drop-down list, click **Add an account**, and then proceed through the sign-in process.
5. After you've signed in with your Azure account, the subscription field will be populated automatically and the **BlueYonderWebsite***yourInitials* (replace _yourInitials_ with your initials) folder will appear below the group of fields.
6. Expand the **BlueYonderWebsite***yourInitials* folder, and then select the **BlueYonderWebsite***yourInitials* web app.
7. To start the publication process, click **OK**. It might take a few minutes.
...
8. After the publish process completes, open a browser, and then navigate to **http://blueyonderwebsite***yourInitials*__.azurewebsites.net/api/locations__.
9. An XML or a JSON describing a list of locations appears. (On Chrome - XML, and on Microsoft Edge and Firefox - JSON).
