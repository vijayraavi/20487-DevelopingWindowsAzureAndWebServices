...
20. On the top-left side of the portal, click the **+** button.
...
53. On the top-left side of the portal, click the **+** button.
54. Click **Web & Mobile**.
55. Click **Web App**.
56. In the **App name** text box type **ServiceBusDemo07YourInitials** (Replace _YourInitials_ with your initials).
57. Click **App Service Plan/Location**, and click **Create New**
58. In **App Service Plan** text box type **ServiceBusDemo07YourInitials** (Replace _YourInitials_ with your initials).
59. In the **Location** drop-down list, select the region closest to your location.
60. Click **OK**
61. Click **Create**. Wait until the web app is created.
62. In the **All Resources** box locate you newly created Web App and click it.
63. click **Overview** and then click **Download the publish profile**.
...
27. If you are prompted to allow access to your clipboard, click **Allow access**.
28. Close the dialog box, and then return to Visual Studio 2017.
29. In **Solution Explorer**, under the **ServiceBusMessageSender** project, double-click **App.config**.
30. In the **&lt;appSettings&gt;** section, locate the **Microsoft.ServiceBus.ConnectionString** key.
...
46. Wait for the console prompt **Enter text to send to the queue and press Enter**.
...
55. Check that the console window titled **Receiver** prints the text **Hello** and then **Goodbye**.
...
22. To create the namespace,at the bottom of the dialog box, click the **Create** button, and then wait until the namespace is active.
23. In the **Dashboard**, under the **All Resources** tile, you should see your newly created service bus. Click it.
    >**Note:** If you cannot see the service bus, you can click **See more...** to access the full list of resources, alternatively, you can expand the tile by dragging its bottom-right corner.
24. In the service bus box, click the **Shared access policies** tab on the left-hand menu, then click **RootManageSharedAccessKey**.
25. Point the cursor to the connection string text that starts with **Endpoint=**
26. Click the **Copy** icon to the right side of the text.
27. If you are prompted to allow access to your clipboard, click **Allow access.**
28. Return to Visual Studio 2017.
29. In Solution Explorer, under the **ServiceBusTopicPublisher** project, double-click **App.config**.
30. In the **&lt;appSettings&gt;** section, locate the **Microsoft.ServiceBus.ConnectionString** key.
31. To paste the connection string over the existing connection string placeholder, select the content of the **value** attribute, and then press Ctrl+V.
32. To save the changes, press Ctrl+S.
33. Open the **App.config** file in the **ExpensivePurchasesSubscriber**, **CheapPurchasesSubscriber**, and **AuditSubscriber** projects, and then perform steps 30-32 in each configuration file.
34. In **Solution Explorer**, right-click the **ServiceBusTopicPublisher** project, and then click **Set as StartUp Project**.
35. To start the project without debugging, press Ctrl+F5.
36. Wait until the console prompt shows the message **Press any key to start publishing messages** , and then press Enter.
37. Wait until the console prompt shows the message **Sending complete** , and then press Enter to close the console window.
38. Return to Visual Studio 2017.
39. In **Solution Explorer**, right-click the root node of the solution, and then click **Properties**.
40. In the **Solution &#39;TopicsDemo&#39; Property Pages** dialog box, click **Multiple startup projects**.
41. In the grid view, change the action to **Start** for the following projects:
...
42. Click **OK**.
43. To run the projects, press Ctrl+F5.
44. Wait for the three console windows to open. Show the students that messages are received in the console windows of the subscribers according to the subscription filter defined in the **ServiceBusTopicPublisher** project:
...
45. Close the three console windows.
