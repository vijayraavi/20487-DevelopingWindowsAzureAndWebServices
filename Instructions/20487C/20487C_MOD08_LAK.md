...
13. Locate the **BaseUri** property, and then replace the **{AppService}** string with the name of the Azure App Service you wrote down at the beginning of this lab.
...
20. Wait for the app to show a list of flights from Seattle to New York.
...
30. Verify that the weather forecast does not display the temperature, and displays only the Fahrenheit sign.
31. Close the client app.
32. Go to the Azure portal at http://portal.azure.com.
33. If a page appears prompting for your email address, enter your email address, and then click **Continue**. Wait for the **sign in** page to appear, enter your email address and password, and then click **Sign In**.
...
34. In the navigation pane, click **App Services**, and in the **App Services** list, click the name of the app service you wrote down at the beginning of this lab.
35. In the app service blade, under **Deployment**, click **Deployment slots**.
36. In the **Deployment Slots** blade, click **Staging**.
37. In the **Staging** blade, copy the **URL** value.
38. Return to **Visual Studio 2017**. 
39. In **Solution Explorer**, under the **BlueYonder.Companion.Shared** project, double-click **Addresses.cs**.
40. Switch the comments between the two **BaseUri get** implementations by placing the production URL in the comment and removing the comment from the staging URL. The resulting code should resemble the following:
...
41. In the **BaseUri** property, select the value **{StagingAddress}**, and then, to paste the copied staging deployment address over it, press Ctrl+V.
42. To save the file, press Ctrl+S.
43. To start the client app without debugging, press Ctrl+F5.
44. After the app starts, verify that the weather forecast appears for the current trip.
45. Close the client app.
