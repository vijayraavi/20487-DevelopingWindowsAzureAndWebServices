...
9. Click **Start**, type **Visual Studio**. From the search results, right-click **Visual Studio 2017** and then select **Run as administrator**.
10. On the **File** menu, point to **Open**, and then click **Project/Solution**.
11. Type **[repository root]\AllFiles\20487C\Mod08\LabFiles\begin\BlueYonder.Server\BlueYonder.Companion.sln** in the **File name** text box, and then click **Open**.
12. In **Solution Explorer**, right-click the **BlueYonder.Companion.Host** project, and then click **Publish**.
13. In the **Publish** window, select **Microsoft Azure App Service**, select **Select existing**, and then click **Publish**.
14. In the **App Service** modal, expand the **BlueYonder.Lab.08** node.
15. Select **blueyonder-companion-08-**_yourinitials_, and then click **OK**.
16. Wait for the publish operation to complete, a browser should open automatically in the deployed app service.
17. Return to the **Visual Studio 2017** instance.
18. In **Solution Explorer**, expand the **BlueYonder.Companion.Controllers** project, and then double-click **LocationController.cs**.
19. Locate the **GetWeather** method and replace its code with the following code.
...
21. On the **View** menu, click **Task List**.
22. In **Task List**, select **Comments** from the drop-down list at the top.
23. In **Task List**, double-click the **// TODO: Lab 08: Exercise 1: Task 1.5: Add route for the weather updates** comment. 
24. Verify that the **WebApiConfig.cs** file opens.
25. Add the following code under the comment.
...
5. Click on **Make user you're running with a Standard or Premium plan before adding a slot** message.
6. In **Choose your pricing tier** window, select **S1 Standard**, and then click **Select**.
7. Wait until the process will finish.
8. Click **Add Slot**.
9. In the **Name** text box, type **Staging**.
10. Click **OK**, and then wait until the deployment slot is created.
11. Go back to **Visual Studio 2017**.
12. In **Solution Explorer**, right-click the **BlueYonder.Companion.Host** project, and then click **Publish**.
13. In the **Publish** window, select **Microsoft Azure App Service**, select **Select existing**, and then click **Publish**.
14. In the **App Service** modal, expand the **BlueYonder.Lab.08** node.
15. Expand **blueyonder-companion-08-**_yourinitials_.
16. Expand **Deployment Slots**.
17. Select **Staging**, and then click **OK**.
