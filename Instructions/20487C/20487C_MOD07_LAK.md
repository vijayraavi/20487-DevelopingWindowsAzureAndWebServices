...
3. Locate the address bar abd add **/booking.svc** to the end of your address. Then wait until it will finish to load.
...
10. In **Task List**, click **Entire Solution** in the drop-down list at the top.
11. Double-click the **// TODO: Lab 07 Exercise 1: Task 4.3: Bring back the call to the backend WCF service** comment.
12. Uncomment the call to the **CreateReservationOnBackendSystem** method. Make sure the return value of the method is stored in the **confirmationCode** variable.
13. To save the file, press Ctrl+S.
14. In **Solution Explorer**, right-click the **BlueYonder.Companion.Host**, and then click **Publish**.
15. Select **Microsoft Azure App Service** and make sure **Create New** is selected, and then click **Publish**.
16. In **Create App Service**, type the following values:
     - App Name: **BlueYonderLab07Companion-**_YourInitials_
     - Resource Group: **BlueYonder.Lab.07**
17. Click **Create** and then click **Publish**.
18. Switch to the Visual Studio 2017 instance that has the **BlueYonder.Server** solution open.
19. In **Solution Explorer**, expand the **BlueYonder.BookingService.Implementation** project, and then double-click **BookingService.cs**.
20. In the **CreateReservation** method, right-click the line of code that starts with **if(request.DepartureFlight**, point to **Breakpoint**, and then click **Insert Breakpoint**.
21. In **Solution Explorer**, right-click the **BlueYonder.Server.Booking.WebHost** project, and then click **Set as StartUp Project**.
22. To start debugging the WCF application, press F5.
23. Open another instance of **Visual Studio 2017**.
24. On the **File** menu, point to **Open**, and then click **Project/Solution**.
25. In the **File name** text box, type 
...
26. If the Windows **Settings** window is open in the **For Developers** section, select **Developer mode**. In the dialog box that appears, click **Yes**.
27. In **Solution Explorer**, expand the **BlueYonder.Companion.Shared** project, and then double-click **Addresses.cs**.
28. In the **Addresses** class, locate the **BaseUri** property, and then replace the **[Web Api web app name]** string with the **BlueYonderLab07Companion-**_YourInitials_.
29. To save the changes, press Ctrl+S.
30. To start the client app without debugging, press Ctrl+F5.
31. If you are prompted to allow the app to run in the background, click **Allow**.
32. To display the app bar, right click or swipe from the bottom of the screen.
33. Click **Search**, and then in the **Search** text box, type **New**.
34. If you are prompted to allow the app to share your location, click **Allow**.
35. Wait for the app to show a list of flights from Seattle to New York.
36. Click **Purchase this trip**.
37. In the **First Name** text box, enter your first name.
38. In the **Last Name** text box, enter your last name.
39. In the **Passport** text box, type **Aa1234567**.
40. In the **Mobile Phone** text box, type **555-5555555**.
41. In the **Home Address** text box, type **423 Main St.**.
42. In the **Email Address** text box, enter your email address.
43. Click **Purchase**.
44. Switch to the Visual Studio 2017 instance that has the **BlueYonder.Server** solution open.  
...
48. Return to the Visual Studio 2017 instance where the **BlueYonder.Server** solution is open, and then press Shift+F5 to stop debugging the WCF application.
