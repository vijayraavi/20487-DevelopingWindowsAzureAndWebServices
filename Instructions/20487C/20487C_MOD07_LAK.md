...
21. Click **Create** and then if the App is not published automatically, then click **Publish**.
...
25. Open another instance of **Visual Studio 2017**.
26. On the **File** menu, point to **Open**, and then click **Project/Solution**.
27. In the **File name** text box, type 
...
28. If the Windows **Settings** window is open in the **For Developers** section, select **Developer mode**. In the dialog box that appears, click **Yes**.
29. In **Solution Explorer**, expand the **BlueYonder.Companion.Shared** project, and then double-click **Addresses.cs**.
30. In the **Addresses** class, locate the **BaseUri** property, and then replace the **[Web Api web app name]** string with the **BlueYonderLab07Companion**_YourInitials_.
31. To save the changes, press Ctrl+S.
32. To start the client app without debugging, press Ctrl+F5.
33. If you are prompted to allow the app to run in the background, click **Allow**.
34. To display the app bar, right click or swipe from the bottom of the screen.
35. Click **Search**, and then in the **Search** text box, type **Sea**.
36. If you are prompted to allow the app to share your location, click **Allow**.
37. Wait for the app to show a list of flights from Seattle to New York.
38. Click **Purchase this trip**.
39. In the **First Name** text box, enter your first name.
40. In the **Last Name** text box, enter your last name.
41. In the **Passport** text box, type **Aa1234567**.
42. In the **Mobile Phone** text box, type **555-5555555**.
43. In the **Home Address** text box, type **423 Main St.**.
44. In the **Email Address** text box, enter your email address.
45. Click **Purchase**.
46. Switch to the Visual Studio 2017 instance that has the **BlueYonder.Server** solution open.  
    Note that the code execution breaks, and the line in the breakpoint is highlighted in yellow.
47. To resume execution, press F5.
48. Go back to the client app.
49. To close the confirmation message, click **Close**, and then close the client app.
50. Return to the Visual Studio 2017 instance where the **BlueYonder.Server** solution is open, and then press Shift+F5 to stop debugging the WCF application.
...
24. Place the following code after the **// TODO: Lab07, Exercise 2, Task 1.6 : Send a flight update message to the queue** comment.
    ...
    ```cs
        updatedSchedule.Flight.FlightId = id;
...
4. Select **BlueYonder.Companion.WNS.csproj**, and then click Ok.
...
11. Click **OK**.
12. Add the following references to the **BlueYonder.Companion.Functions** project:
    - **BlueYonder.Companion.Entities**
    - **BlueYonder.Entities**
    - **BlueYonder.Companion.Entities.Mappers**
13. Right-click the new project you just created.
14. Point to **Add**, and then click **New Item**.
15. Select **Azure Function**.
16. Name the function **PublishFlightUpdate**.   
17. Click **OK**, a **New Azure Function - PublishFlightUpdate** window opens.
18. In the list on the left, select **Service Bus Queue trigger**.
19. In **Access Rights**, select **Manage**.
20. In the **Connection** text box, type **ServiceBusConnectionString**.
21. In the **Queue name** text box, type **FlightUpdatesQueue**.
22. Click **Ok**.  
    A new file **PublishFlightUpdates.cs** is added to the **BlueYonder.Companion.Functions** project.
23. Add the following **using** statements:
    ```cs
        using BlueYonder.Companion.Entities;
        using BlueYonder.Entities;
        using BlueYonder.Companion.Entities.Mappers;
     ```
24. Change the signature of the **Run** method as follows:
    ```cs
    ...
    ```
25. Now add the following line to the **Run** method:
    ```cs
    MessageHandler.Publish(updatedScheduleDto.ToFlightSchedule());
    ```
26. Right-click the **BlueYonder.Companion.Functions** project.
27. Point to **Add**, and then click **Existing Item**.
28. Select **MessageHandler.cs** that is located in **[repository root]\AllFiles\20487C\Mod07\LabFiles\Assets\MessageHandler.cs** and then, click **Ok**.
29. Under **BlueYonder.Companion.Functions**, right-click **References**, and then click **Add Reference**.
...
1. Right click the **BlueYonder.Companion.Functions** project, and then click **Publish**.  
   ...
   Select **Azure Functions App**, make sure **Create New** is selected, and then click **Create Profile**
