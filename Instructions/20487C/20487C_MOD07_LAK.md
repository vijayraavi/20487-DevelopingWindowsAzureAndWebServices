...
1. Open the Microsoft Power Shell window.
2. Go to **[repository root]\AllFiles\20487C\Mod07\LabFiles\Setup** (For example, if the repository root is in **C:\Users\JohnDoe\Azure**, change the path to **C:\Users\JohnDoe\Azure\AllFiles\20487C\Mod07\LabFiles\Setup**).
3. Run **ps createAzureServices.ps1**. 
4. You are prompted to supply a subscription ID, which you can get by performing the following steps:
    ...
    b. In the search "text box on the top bar, type **Subscriptions**" and then press Enter. The **Cost Management + Billing - Subscriptions** window should appears.
...
5. In the Sign in window that appears, enter your details, and sign in.
6. When the deployment ends, copy the output connection strings to a text file for later use.
...
8. In the **Create Namespace** dialog box provide the following information:
    ...
    To create the namespace, click the **Create** button.
9. Wait until the namespace is created. 
...
4. In **Solution Explorer**, right click the **BlueYonder.Server.Booking.WebHost** project, and click then **Manage NuGet packages**.
...
1. On the Start screen, right-click the **Visual Studio 2017** tile, and then on the app bar, click **Open new window**.
2. On the **File** menu, point to **Open**, and then click **Project/Solution**.
...
43. Note that the code execution breaks, and the line in the breakpoint is highlighted in yellow.
44. To resume execution, press F5.
45. Go back to the client app.
46. To close the confirmation message, click **Close**, and then close the client app.
47. Return to the Visual Studio 2017 instance where the **BlueYonder.Server** solution is open, and then press Shift+F5 to stop debugging the WCF application.
...
9. Return to the Visual Studio 2017 instance where the **BlueYonder.Companion** solution is open.
...
13. In the **Package SID** text box, type **"ms-app://s-1-15-2-1943958206-46993560-2941426192-3413777431-2244800519-641792188-4204354165"**.
14. In the **Security Key** text box, type **"/A6E8J9JmfDFZMX5lhuCJdTeI32zcXMM"**.
15. Click **Save**.
...
15. Name the function "**PublishFlightUpdate**".
    Click **OK**
16. A "**New Azure Function - PublishFlightUpdate**" window opens.
...
30. On the menu on the left click **Solution**.
31. Select **BlueYonder.Companion.WNS**, and then click **Ok**.
...
4. In the **Resource Group** field, if nothing is populated, click **New**, enter a name to identify the resource group, and then click **OK**.
5. In **App Service Plan** field is empty, click **New**:
...
3. To run the application without debugging, press Ctrl + F5.  
