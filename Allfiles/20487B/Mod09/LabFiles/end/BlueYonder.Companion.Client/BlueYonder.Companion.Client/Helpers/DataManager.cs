...
namespace BlueYonder.Companion.Client.Helpers
{
    public class DataManager
    {
        ...
        public async Task<IEnumerable<File>> GetAzureStorageFiles(int reservationId)
        {
        ...
        public async Task<IEnumerable<File>> GetAzurePublicStorageFilesByLocation(int reservationId)
        {
            var uri = new Uri(string.Format(Addresses.GetLocationMetadataUri, reservationId));
            var response = await GetAsync(uri);
            var fileDtos = JsonSerializerHelper.Deserialize<IEnumerable<FileDTO>>(response.Content);
            var files = fileDtos.Select(fileDto => fileDto.ToObject());
            return files;
        ...
        public async Task<bool> CreateAzureStorageFileMetadata(string fileUri, string fileName, int reservationId, bool isPrivate, int travelerId)
        {
